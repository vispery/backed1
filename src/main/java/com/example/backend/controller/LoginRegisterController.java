    ...
    public Map<String, Object> register(@RequestBody String json) {
        ...
        if (response.get("ok").toString().equals("true")) {
            ...
            String content = "尊敬的用户：\n\n" +
                    ...
                    projectProperties.getBackendRootUrl() + "active_account?id=" + tmpTenant.getId() + "&active_code=" + uuid + "\n\n" +
                    "\t如果确定是您本人操作，请访问下方的链接，以完成注册过程。\n\n" +
                    "\t如果不是您本人操作，请忽视该邮件，给您带来的不便请谅解！";
            mailService.sendSimpleMail(email, "Free租房网站注册验证邮件", content);
        }
        return response.toMap();
    }
